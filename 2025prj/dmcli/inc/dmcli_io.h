...
#define DEFAULT_IO_PROMPT_CAP 128
...
struct dmcli_io{
    ...
    char prompt[DEFAULT_IO_PROMPT_CAP];          // Prompt string (shows the user when to input data).
    size_t prompt_capacity; // Prompt string maximum capacity in characters.
    size_t prompt_length;   // Pormpt string length in characters.

    char input[DEFAULT_IO_INPUT_CAP];           // User input, stores the most recent user input data.
    ...
    char ilog[DEFAULT_IO_ILOG_CAP][DEFAULT_IO_INPUT_CAP];           // Input log, stores a historical of the user input.
